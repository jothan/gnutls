		...
		if (res) {
			if (non_printable == 0) {
				...
				*res_size = len;
				...
				result =
				    _gnutls_x509_data2hex(str, len, res,
							  res_size);
				if (result < 0) {
					gnutls_assert();
					return result;
...
int _gnutls_x509_set_time(ASN1_TYPE c2, const char *where, time_t tim)
{
	...
	result =
	    _gnutls_x509_gtime2utcTime(tim, str_time, sizeof(str_time));
	if (result < 0) {
		gnutls_assert();
		return result;
	}
	...
	if (format == GNUTLS_X509_FMT_DER) {
		...
		*output_data_size = len;
		...
		if ((result =
			...
			if (result == ASN1_MEM_ERROR) {
				*output_data_size =
				    B64FSIZE(strlen(pem_header), len) + 1;
			}
			gnutls_afree(tmp);
			return _gnutls_asn2err(result);
		}
				     ...
				     int str)
{
	 ...
	 */
	result =
	    asn1_write_value(dest, dest_name, encoded.data, encoded.size);
					    ...
					    int params_size)
{
	...
	if (pk_algorithm == GNUTLS_PK_RSA) {
		...
		result =
		    _gnutls_x509_write_rsa_params(params, params_size,
						  &der);
		if (result < 0) {
			gnutls_assert();
			return result;
		...
		result =
		    asn1_write_value(dst, name, der.data, der.size * 8);
		...
		result =
		    _gnutls_x509_write_dsa_params(params, params_size,
						  &der);
		if (result < 0) {
			gnutls_assert();
			return result;
		...
		result =
		    _gnutls_x509_write_dsa_public_key(params, params_size,
						      &der);
		if (result < 0) {
			gnutls_assert();
			return result;
